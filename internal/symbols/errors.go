...
func NodeError(node ast.Node, format string, a ...any) ParserError {
	return ParserError{node, fmt.Sprintf(format, a...)}
}

func UnknownSelector(node ast.Node, selector string) ParserError {
	return NodeError(node, "unknown selector %s", selector)
}

func AmbiguousObjectError(node ast.Node, obj Object) ParserError {
	return NodeError(node, "cannot evaluate ambiguous object %T", obj)
}

func NoPropertyError(node ast.Node, resolveChain string, obj Object, key string) ParserError {
...
func NotIndexableError(node ast.Node, resolveChain string, obj Object) ParserError {
	if valueObj, ok := obj.(ValueObject); ok {
...
func NotIterableError(node ast.Node, obj Object) ParserError {
	if valueObj, ok := obj.(ValueObject); ok {
...
func UncallableError(node ast.Node, resolveChain string, obj Object) ParserError {
	if valueObj, ok := obj.(ValueObject); ok {
...
func InvalidIndexError(node ast.Node, obj Object) ParserError {
	return NodeError(node, "got %T for index, expected Integer", obj)
}

func InvalidTypeError(node ast.Node, obj Object) ParserError {
	return NodeError(node, "cannot use %T for type", obj)
}

func InvalidArgumentLengthError(node ast.Node, expected, got []Object) ParserError {
	return NodeError(node, "expected %d arguments, got %d", len(expected), len(got))
}

func InvalidReturnTypeError(node ast.Node, obj Class, class Class) ParserError {
	return NodeError(node, "return type %s does not match expected %s", obj.ClassName(), class.ClassName())
}

func InoperableSwitchTargetError(node ast.Node, obj Object) ParserError {
	if valueObj, ok := obj.(ValueObject); ok {
...
func UnknownInterfaceError(node ast.Node, className string) ParserError {
	return NodeError(node, "unknown interface %s", className)
}

func InvalidValueExpressionError(node ast.Node) ParserError {
